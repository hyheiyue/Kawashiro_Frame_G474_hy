...
arm_absmax_q31(const q31_t* pSrc, uint32_t blockSize, q31_t* pResult, uint32_t* pIndex) {
...
arm_absmax_q31(const q31_t* pSrc, uint32_t blockSize, q31_t* pResult, uint32_t* pIndex) {
    ...
    out = *pSrc++;
    out = (out > 0) ? out : (q31_t)__QSUB(0, out); /* Initialize index of extrema value. */
    index = 0U;
    ...
    while (blkCnt > 0U) { /* Initialize cur_absmax to next consecutive values one by one */
        ...
        cur_absmax = (cur_absmax > 0)
            ? cur_absmax
            : (q31_t)__QSUB(0, cur_absmax); /* compare for the extrema value */
