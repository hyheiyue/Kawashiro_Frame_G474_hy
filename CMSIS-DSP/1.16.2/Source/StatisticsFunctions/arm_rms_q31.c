...
ARM_DSP_ATTRIBUTE void arm_rms_q31(const q31_t* pSrc, uint32_t blockSize, q31_t* pResult) {
    q63_t pow = 0.0f;
...
ARM_DSP_ATTRIBUTE void arm_rms_q31(const q31_t* pSrc, uint32_t blockSize, q31_t* pResult) {
    ...
    uint64_t sum =
        0; /* Temporary result storage (can get never negative. changed type from q63 to uint64 */
