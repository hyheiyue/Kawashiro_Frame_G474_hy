...
ARM_DSP_ATTRIBUTE void arm_quaternion_normalize_f32(
...
ARM_DSP_ATTRIBUTE void arm_quaternion_normalize_f32(
    ...
    for (i = 0; i < nbQuaternions; i++) {
        temp = ARM_SQ(pInputQuaternions[4 * i + 0]) + ARM_SQ(pInputQuaternions[4 * i + 1])
            + ARM_SQ(pInputQuaternions[4 * i + 2]) + ARM_SQ(pInputQuaternions[4 * i + 3]);
        temp = sqrtf(temp);
