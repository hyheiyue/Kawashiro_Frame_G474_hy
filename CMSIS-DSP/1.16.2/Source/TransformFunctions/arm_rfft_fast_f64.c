...
static void
stage_rfft_f64(const arm_rfft_fast_instance_f64* S, const float64_t* p, float64_t* pOut) {
...
static void
merge_rfft_f64(const arm_rfft_fast_instance_f64* S, const float64_t* p, float64_t* pOut) {
    uint32_t k; /* Loop Counter */
    float64_t twR, twI; /* RFFT Twiddle coefficients */
    const float64_t* pCoeff = S->pTwiddleRFFT; /* Points to RFFT Twiddle factors */
    const float64_t* pA = p; /* increasing pointer */
...
arm_rfft_fast_f64(arm_rfft_fast_instance_f64* S, float64_t* p, float64_t* pOut, uint8_t ifftFlag) {
