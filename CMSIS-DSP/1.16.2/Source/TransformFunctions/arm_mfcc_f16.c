...
arm_mfcc_f16(const arm_mfcc_instance_f16* S, float16_t* pSrc, float16_t* pDst, float16_t* pTmp) {
    float16_t maxValue;
    uint32_t index;
    uint32_t i;
    float16_t result;
  ...
  */
    ...
    for (i = 0; i < S->nbMelFilters; i++) {
        arm_dot_prod_f16(pSrc + S->filterPos[i], coefs, S->filterLengths[i], &result);
