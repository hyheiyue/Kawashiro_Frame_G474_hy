...
arm_mat_vec_mult_q31(const arm_matrix_instance_q31* pSrcMat, const q31_t* pSrcVec, q31_t* pDstVec) {
...
#else
ARM_DSP_ATTRIBUTE void
arm_mat_vec_mult_q31(const arm_matrix_instance_q31* pSrcMat, const q31_t* pVec, q31_t* pDst) {
    uint32_t numRows = pSrcMat->numRows;
    uint32_t numCols = pSrcMat->numCols;
