...
ARM_DSP_ATTRIBUTE arm_status arm_mat_ldlt_f32(
    ...
    if ((pSrc->numRows != pSrc->numCols) || (pl->numRows != pl->numCols)
        || (pd->numRows != pd->numCols) || (pl->numRows != pd->numRows))
    {
        ...
        for (k = 0; k < n; k++) {
            ...
            for (w = k + 1; w < n; w += 4) {
                ...
                for (int x = k + 1; x < n; x += 4) {
                    ...
                    vecX =
                        vldrwq_gather_shifted_offset_z_f32(&pA[x * n + k], (uint32x4_t)vecOffs, p0);
