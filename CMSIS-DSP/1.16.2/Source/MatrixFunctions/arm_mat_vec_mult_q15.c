...
arm_mat_vec_mult_q15(const arm_matrix_instance_q15* pSrcMat, const q15_t* pSrcVec, q15_t* pDstVec) {
...
#else
ARM_DSP_ATTRIBUTE void
arm_mat_vec_mult_q15(const arm_matrix_instance_q15* pSrcMat, const q15_t* pVec, q15_t* pDst) {
    uint32_t numRows = pSrcMat->numRows;
    uint32_t numCols = pSrcMat->numCols;
