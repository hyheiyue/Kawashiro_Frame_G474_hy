...
/** @defgroup UARTEx_Private_Functions UARTEx Private Functions
  ...
  */
static void
UARTEx_Wakeup_AddressConfig(UART_HandleTypeDef* huart, UART_WakeUpTypeDef WakeUpSelection);
...
/**
  ...
  */
HAL_StatusTypeDef
HAL_MultiProcessorEx_AddressLength_Set(UART_HandleTypeDef* huart, uint32_t AddressLength) {
    /* Check the UART handle allocation */
    if (huart == NULL) {
        return HAL_ERROR;
    }
...
HAL_StatusTypeDef HAL_UARTEx_StopModeWakeUpSourceConfig(
    ...
    if (UART_WaitOnFlagUntilTimeout(
            huart,
            USART_ISR_REACK,
            RESET,
            tickstart,
