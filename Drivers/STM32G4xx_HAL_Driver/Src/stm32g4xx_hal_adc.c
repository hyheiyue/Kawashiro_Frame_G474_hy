...
HAL_StatusTypeDef HAL_ADC_Init(ADC_HandleTypeDef* hadc) {
        ...
        && (tmp_adc_is_conversion_on_going_regular == 0UL))
    {
        ...
        tmp_cfgr =
            (ADC_CFGR_CONTINUOUS((uint32_t)hadc->Init.ContinuousConvMode) | hadc->Init.Overrun
             | hadc->Init.DataAlign | hadc->Init.Resolution
...
HAL_StatusTypeDef HAL_ADC_DeInit(ADC_HandleTypeDef* hadc) {
    ...
    __HAL_ADC_DISABLE_IT(
        hadc,
        (ADC_IT_AWD3 | ADC_IT_AWD2 | ADC_IT_AWD1 | ADC_IT_JQOVF | ADC_IT_OVR | ADC_IT_JEOS
    ...
    CLEAR_BIT(
        hadc->Instance->CFGR2,
        ADC_CFGR2_ROVSM | ADC_CFGR2_TROVS | ADC_CFGR2_OVSS | ADC_CFGR2_OVSR | ADC_CFGR2_JOVSE
            | ADC_CFGR2_ROVSE
    );
            ...
            && (tmp_adc_is_conversion_on_going_injected == 0UL))
        {
            ...
            if (pConfig->OffsetNumber != ADC_OFFSET_NONE) {
                ...
                LL_ADC_SetOffsetSaturation(
                    hadc->Instance,
                    pConfig->OffsetNumber,
                    (pConfig->OffsetSaturation == ENABLE) ? LL_ADC_OFFSET_SATURATION_ENABLE
...
HAL_ADC_AnalogWDGConfig(ADC_HandleTypeDef* hadc, const ADC_AnalogWDGConfTypeDef* pAnalogWDGConfig) {
        ...
        && (tmp_adc_is_conversion_on_going_injected == 0UL))
    {
        ...
        if (pAnalogWDGConfig->WatchdogNumber == ADC_ANALOGWATCHDOG_1) {
            ...
            MODIFY_REG(hadc->Instance->TR1, ADC_TR1_AWDFILT, pAnalogWDGConfig->FilteringConfig);
