...
/** @defgroup FDCAN FDCAN
  ...
  */
        ...
        #define SRAMCAN_FLESA \
            ((uint32_t)(SRAMCAN_FLSSA + (SRAMCAN_FLS_NBR * SRAMCAN_FLS_SIZE)) \
            ) /* Filter List Extended Start
                                                                                            Address                  */
        #define SRAMCAN_RF0SA \
            ((uint32_t)(SRAMCAN_FLESA + (SRAMCAN_FLE_NBR * SRAMCAN_FLE_SIZE)) \
            ) /* Rx FIFO 0 Start Address  */
        #define SRAMCAN_RF1SA \
            ((uint32_t)(SRAMCAN_RF0SA + (SRAMCAN_RF0_NBR * SRAMCAN_RF0_SIZE)) \
            ) /* Rx FIFO 1 Start Address  */
        #define SRAMCAN_TEFSA \
            ((uint32_t)(SRAMCAN_RF1SA + (SRAMCAN_RF1_NBR * SRAMCAN_RF1_SIZE)) \
            ) /* Tx Event FIFO Start
                                                                                            Address */
        #define SRAMCAN_TFQSA \
            ((uint32_t)(SRAMCAN_TEFSA + (SRAMCAN_TEF_NBR * SRAMCAN_TEF_SIZE)) \
            ) /* Tx FIFO/Queue Start
                                                                                            Address                  */
        #define SRAMCAN_SIZE \
...
HAL_StatusTypeDef HAL_FDCAN_UnRegisterTxBufferCompleteCallback(FDCAN_HandleTypeDef* hfdcan) {
    ...
    if (hfdcan->State == HAL_FDCAN_STATE_READY) {
        hfdcan->TxBufferCompleteCallback =
            HAL_FDCAN_TxBufferCompleteCallback; /* Legacy weak TxBufferCompleteCallback  */
    } else {
        /* Update the error code */
        hfdcan->ErrorCode |= HAL_FDCAN_ERROR_INVALID_CALLBACK;
...
HAL_FDCAN_ConfigFilter(FDCAN_HandleTypeDef* hfdcan, const FDCAN_FilterTypeDef* sFilterConfig) {
    ...
    if ((state == HAL_FDCAN_STATE_READY) || (state == HAL_FDCAN_STATE_BUSY)) {
        ...
        if (sFilterConfig->IdType == FDCAN_STANDARD_ID) {
            /* Check function parameters */
            assert_param(
                IS_FDCAN_MAX_VALUE((sFilterConfig->FilterIndex + 1U), hfdcan->Init.StdFiltersNbr)
            );
            ...
            FilterElementW1 =
                ((sFilterConfig->FilterType << 30U) | (sFilterConfig->FilterConfig << 27U)
                 | (sFilterConfig->FilterID1 << 16U) | sFilterConfig->FilterID2);

            /* Calculate filter address */
            FilterAddress =
                (uint32_t*)(hfdcan->msgRam.StandardFilterSA + (sFilterConfig->FilterIndex * SRAMCAN_FLS_SIZE));
            ...
            *FilterAddress = FilterElementW1;
            ...
            /* Check function parameters */
            assert_param(
                IS_FDCAN_MAX_VALUE((sFilterConfig->FilterIndex + 1U), hfdcan->Init.ExtFiltersNbr)
            );
            ...
            /* Calculate filter address */
            FilterAddress =
                (uint32_t*)(hfdcan->msgRam.ExtendedFilterSA + (sFilterConfig->FilterIndex * SRAMCAN_FLE_SIZE));
...
static void FDCAN_CalcultateRamBlockAddresses(FDCAN_HandleTypeDef* hfdcan) {
    ...
    for (RAMcounter = SramCanInstanceBase; RAMcounter < (SramCanInstanceBase + SRAMCAN_SIZE);
         RAMcounter += 4U)
    {
...
static void FDCAN_CopyMessageToRAM(
    ...
    if (pTxHeader->IdType == FDCAN_STANDARD_ID) {
        TxElementW1 =
            (pTxHeader->ErrorStateIndicator | FDCAN_STANDARD_ID | pTxHeader->TxFrameType
             | (pTxHeader->Identifier << 18U));
    } else /* pTxHeader->IdType == FDCAN_EXTENDED_ID */ {
        TxElementW1 =
            (pTxHeader->ErrorStateIndicator | FDCAN_EXTENDED_ID | pTxHeader->TxFrameType
             | pTxHeader->Identifier);
    }

    /* Build second word of Tx header element */
    TxElementW2 =
        ((pTxHeader->MessageMarker << 24U) | pTxHeader->TxEventFifoControl | pTxHeader->FDFormat
    ...
    *TxAddress = TxElementW2;
    ...
    for (ByteCounter = 0; ByteCounter < DLCtoBytes[pTxHeader->DataLength]; ByteCounter += 4U) {
        *TxAddress =
            (((uint32_t)pTxData[ByteCounter + 3U] << 24U)
