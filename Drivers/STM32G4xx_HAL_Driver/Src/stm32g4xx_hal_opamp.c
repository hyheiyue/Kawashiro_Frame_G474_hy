...
/** @addtogroup STM32G4xx_HAL_Driver
  ...
  */
        ...
        #define OPAMP_CSR_UPDATE_PARAMETERS_INIT_MASK \
            (OPAMP_CSR_TRIMOFFSETN | OPAMP_CSR_TRIMOFFSETP | OPAMP_CSR_HIGHSPEEDEN \
             | OPAMP_CSR_OPAMPINTEN | OPAMP_CSR_PGGAIN | OPAMP_CSR_VPSEL | OPAMP_CSR_VMSEL \
             | OPAMP_CSR_FORCEVP)
        /* TCMR init register Mask */
        #define OPAMP_TCMR_UPDATE_PARAMETERS_INIT_MASK \
            (OPAMP_TCMR_T20CMEN | OPAMP_TCMR_T8CMEN | OPAMP_TCMR_T1CMEN | OPAMP_TCMR_VPSSEL \
             | OPAMP_TCMR_VMSSEL)
/**
  * @}
  */
...
HAL_StatusTypeDef HAL_OPAMP_Init(OPAMP_HandleTypeDef* hopamp) {
    ...
    if (hopamp == NULL) {
        ...
        if ((hopamp->Init.Mode == OPAMP_PGA_MODE) || (hopamp->Init.Mode == OPAMP_FOLLOWER_MODE)) {
            ...
            MODIFY_REG(
                ...
                hopamp->Init.PowerMode | hopamp->Init.Mode | hopamp->Init.NonInvertingInput
                    ...
                    | hopamp->Init.PgaConnect | hopamp->Init.PgaGain
        ...
        if ((READ_BIT(hopamp->Instance->TCMR, OPAMP_TCMR_LOCK)) == 0UL) {
            MODIFY_REG(
                hopamp->Instance->TCMR,
                OPAMP_TCMR_UPDATE_PARAMETERS_INIT_MASK,
                hopamp->Init.TimerControlledMuxmode | hopamp->Init.InvertingInputSecondary
...
HAL_OPAMP_GetTrimOffset(OPAMP_HandleTypeDef* hopamp, uint32_t trimmingoffset) {
    ...
    if (hopamp == NULL) {
        ...
        if ((READ_BIT(hopamp->Instance->CSR, OPAMP_CSR_USERTRIM)) != 0UL) {
            ...
            oldtrimmingvaluep =
                (hopamp->Instance->CSR & OPAMP_CSR_TRIMOFFSETP) >> OPAMP_INPUT_NONINVERTING;
            oldtrimmingvaluen =
                (hopamp->Instance->CSR & OPAMP_CSR_TRIMOFFSETN) >> OPAMP_INPUT_INVERTING;
        }
