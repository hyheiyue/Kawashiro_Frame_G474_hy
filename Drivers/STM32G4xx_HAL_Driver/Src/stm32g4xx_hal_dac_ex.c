...
HAL_StatusTypeDef HAL_DACEx_DualStart(DAC_HandleTypeDef* hdac) {
    ...
    wait_loop_index =
        ((DAC_DELAY_STARTUP_US / 10UL) * ((SystemCoreClock / (100000UL * 2UL)) + 1UL));
...
HAL_StatusTypeDef HAL_DACEx_SawtoothWaveGenerate(
    ...
    if (Channel == DAC_CHANNEL_1) {
        ...
        MODIFY_REG(
            ...
            (StepData << DAC_STR1_STINCDATA1_Pos) | Polarity
        ...
        MODIFY_REG(
            ...
            (StepData << DAC_STR2_STINCDATA2_Pos) | Polarity
...
HAL_StatusTypeDef HAL_DACEx_SawtoothWaveDataReset(DAC_HandleTypeDef* hdac, uint32_t Channel) {
    ...
    if (((hdac->Instance->STMODR >> (Channel & 0x10UL)) & DAC_STMODR_STRSTTRIGSEL1)
        == 0UL /* SW TRIGGER */)
    {
        /* Change DAC state */
        hdac->State = HAL_DAC_STATE_BUSY;
...
HAL_StatusTypeDef HAL_DACEx_SawtoothWaveDataStep(DAC_HandleTypeDef* hdac, uint32_t Channel) {
    ...
    if (((hdac->Instance->STMODR >> (Channel & 0x10UL)) & DAC_STMODR_STINCTRIGSEL1)
        == 0UL /* SW TRIGGER */)
    {
        /* Change DAC state */
        hdac->State = HAL_DAC_STATE_BUSY;
...
HAL_StatusTypeDef HAL_DACEx_SelfCalibrate(
    ...
    if ((hdac == NULL) || (sConfig == NULL)) {
        ...
        while (delta != 0UL) {
            ...
            wait_loop_index =
                ((DAC_DELAY_TRIM_US / 10UL) * ((SystemCoreClock / (100000UL * 2UL)) + 1UL));
            while (wait_loop_index != 0UL) {
                wait_loop_index--;
            }
