...
typedef struct {
    uint32_t
        LastErrorCode; /*!< Specifies the type of the last error that occurred on the FDCAN bus.
                                   This parameter can be a value of @ref FDCAN_protocol_error_code             */

    uint32_t
        DataLastErrorCode; /*!< Specifies the type of the last error that occurred in the data phase
...
typedef void (*pFDCAN_ErrorStatusCallbackTypeDef
  ...
  */
    #define FDCAN_BRS_OFF \
        ((uint32_t)0x00000000U) /*!< FDCAN frames transmitted/received without bit rate switching */
    #define FDCAN_BRS_ON \
        ((uint32_t)0x00100000U) /*!< FDCAN frames transmitted/received with bit rate switching    */
    /**
  * @}
  */
  ...
  */
    ...
    #define FDCAN_FILTER_TO_RXFIFO0_HP \
        ((uint32_t)0x00000005U) /*!< Set high priority and store in FIFO 0 if filter matches   */
    #define FDCAN_FILTER_TO_RXFIFO1_HP \
        ((uint32_t)0x00000006U) /*!< Set high priority and store in FIFO 1 if filter matches   */
    /**
  * @}
  */
  ...
  */
    #define FDCAN_IT_LIST_RX_FIFO0 \
        (FDCAN_IT_RX_FIFO0_MESSAGE_LOST | FDCAN_IT_RX_FIFO0_FULL | FDCAN_IT_RX_FIFO0_NEW_MESSAGE \
    ...
    #define FDCAN_IT_LIST_SMSG \
        (FDCAN_IT_TX_ABORT_COMPLETE | FDCAN_IT_TX_COMPLETE | FDCAN_IT_RX_HIGH_PRIORITY_MSG \
        ) /*!< Status Message Interrupts List     */
    #define FDCAN_IT_LIST_TX_FIFO_ERROR \
        (FDCAN_IT_TX_EVT_FIFO_ELT_LOST | FDCAN_IT_TX_EVT_FIFO_FULL | FDCAN_IT_TX_EVT_FIFO_NEW_DATA \
         | FDCAN_IT_TX_FIFO_EMPTY) /*!< TX FIFO Error Interrupts List      */
    #define FDCAN_IT_LIST_MISC \
        (FDCAN_IT_TIMEOUT_OCCURRED | FDCAN_IT_RAM_ACCESS_FAILURE | FDCAN_IT_TIMESTAMP_WRAPAROUND \
        ) /*!< Misc. Interrupts List              */
    #define FDCAN_IT_LIST_BIT_LINE_ERROR \
        (FDCAN_IT_ERROR_PASSIVE | FDCAN_IT_ERROR_LOGGING_OVERFLOW \
...
HAL_StatusTypeDef
HAL_FDCAN_UnRegisterCallback(FDCAN_HandleTypeDef* hfdcan, HAL_FDCAN_CallbackIDTypeDef CallbackID);
...
HAL_StatusTypeDef
HAL_FDCAN_ConfigFilter(FDCAN_HandleTypeDef* hfdcan, const FDCAN_FilterTypeDef* sFilterConfig);
...
HAL_StatusTypeDef
HAL_FDCAN_ConfigRamWatchdog(FDCAN_HandleTypeDef* hfdcan, uint32_t CounterStartValue);
HAL_StatusTypeDef
HAL_FDCAN_ConfigTimestampCounter(FDCAN_HandleTypeDef* hfdcan, uint32_t TimestampPrescaler);
HAL_StatusTypeDef
HAL_FDCAN_EnableTimestampCounter(FDCAN_HandleTypeDef* hfdcan, uint32_t TimestampOperation);
...
HAL_StatusTypeDef
HAL_FDCAN_GetTxEvent(FDCAN_HandleTypeDef* hfdcan, FDCAN_TxEventFifoTypeDef* pTxEvent);
...
uint32_t
HAL_FDCAN_IsTxBufferMessagePending(const FDCAN_HandleTypeDef* hfdcan, uint32_t TxBufferIndex);
...
HAL_StatusTypeDef
HAL_FDCAN_DeactivateNotification(FDCAN_HandleTypeDef* hfdcan, uint32_t InactiveITs);
  ...
  */
    ...
    #define IS_FDCAN_MODE(MODE) \
        (((MODE) == FDCAN_MODE_NORMAL) || ((MODE) == FDCAN_MODE_RESTRICTED_OPERATION) \
         || ((MODE) == FDCAN_MODE_BUS_MONITORING) || ((MODE) == FDCAN_MODE_INTERNAL_LOOPBACK) \
    ...
    #define IS_FDCAN_TIMESTAMP_PRESCALER(PRESCALER) \
        (((PRESCALER) == FDCAN_TIMESTAMP_PRESC_1) || ((PRESCALER) == FDCAN_TIMESTAMP_PRESC_2) \
         || ((PRESCALER) == FDCAN_TIMESTAMP_PRESC_3) || ((PRESCALER) == FDCAN_TIMESTAMP_PRESC_4) \
         || ((PRESCALER) == FDCAN_TIMESTAMP_PRESC_5) || ((PRESCALER) == FDCAN_TIMESTAMP_PRESC_6) \
         || ((PRESCALER) == FDCAN_TIMESTAMP_PRESC_7) || ((PRESCALER) == FDCAN_TIMESTAMP_PRESC_8) \
         || ((PRESCALER) == FDCAN_TIMESTAMP_PRESC_9) || ((PRESCALER) == FDCAN_TIMESTAMP_PRESC_10) \
         || ((PRESCALER) == FDCAN_TIMESTAMP_PRESC_11) || ((PRESCALER) == FDCAN_TIMESTAMP_PRESC_12) \
         || ((PRESCALER) == FDCAN_TIMESTAMP_PRESC_13) || ((PRESCALER) == FDCAN_TIMESTAMP_PRESC_14) \
         || ((PRESCALER) == FDCAN_TIMESTAMP_PRESC_15) \
         || ((PRESCALER) == FDCAN_TIMESTAMP_PRESC_16))
    #define IS_FDCAN_TIMEOUT(OPERATION) \
        (((OPERATION) == FDCAN_TIMEOUT_CONTINUOUS) || ((OPERATION) == FDCAN_TIMEOUT_TX_EVENT_FIFO) \
         || ((OPERATION) == FDCAN_TIMEOUT_RX_FIFO0) || ((OPERATION) == FDCAN_TIMEOUT_RX_FIFO1))
