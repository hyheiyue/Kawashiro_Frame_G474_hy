...
typedef struct {
    ...
    uint32_t
        TransferDirection; /*!< Specifies whether the Receive and/or Transmit mode is enabled or disabled.
    ...
    uint32_t
        HardwareFlowControl; /*!< Specifies whether the hardware flow control mode is enabled or disabled.
...
typedef struct {
  ...
  */
    ...
    #define LL_USART_BINARY_LOGIC_NEGATIVE \
        USART_CR2_DATAINV /*!< Logical data from the data register are send/received in negative/inverse logic. (1=L, 0=H). The parity bit is also inverted. */
    /**
  * @}
  */
  ...
  */
    ...
    #define LL_USART_AUTOBAUD_DETECT_ON_FALLINGEDGE \
        USART_CR2_ABRMODE_0 /*!< Falling edge to falling edge measurement. Received frame must start with a single bit = 1 -> Frame = Start10xxxxxx */
...
LL_USART_ConfigFIFOsThreshold(USART_TypeDef* USARTx, uint32_t TXThreshold, uint32_t RXThreshold) {
    ATOMIC_MODIFY_REG(
        USARTx->CR3,
...
/**
  ...
  */
__STATIC_INLINE void
LL_USART_SetLastClkPulseOutput(USART_TypeDef* USARTx, uint32_t LastBitClockPulse) {
...
/**
  ...
  */
__STATIC_INLINE void
LL_USART_ConfigNodeAddress(USART_TypeDef* USARTx, uint32_t AddressLen, uint32_t NodeAddress) {
    MODIFY_REG(
        USARTx->CR2,
        USART_CR2_ADD | USART_CR2_ADDM7,
...
/**
  ...
  */
__STATIC_INLINE void
LL_USART_SetSmartcardAutoRetryCount(USART_TypeDef* USARTx, uint32_t AutoRetryCount) {
...
__STATIC_INLINE uint32_t LL_USART_IsEnabledIT_RXNE_RXFNE(const USART_TypeDef* USARTx) {
    return (
        (READ_BIT(USARTx->CR1, USART_CR1_RXNEIE_RXFNEIE) == (USART_CR1_RXNEIE_RXFNEIE)) ? 1UL : 0UL
    );
}
...
__STATIC_INLINE uint32_t LL_USART_IsEnabledIT_TXE_TXFNF(const USART_TypeDef* USARTx) {
    return (
        (READ_BIT(USARTx->CR1, USART_CR1_TXEIE_TXFNFIE) == (USART_CR1_TXEIE_TXFNFIE)) ? 1UL : 0UL
    );
}
...
ErrorStatus
LL_USART_ClockInit(USART_TypeDef* USARTx, const LL_USART_ClockInitTypeDef* USART_ClockInitStruct);
