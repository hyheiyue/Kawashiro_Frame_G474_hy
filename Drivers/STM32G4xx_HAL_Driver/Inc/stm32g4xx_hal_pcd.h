...
HAL_StatusTypeDef
HAL_PCD_EP_Open(PCD_HandleTypeDef* hpcd, uint8_t ep_addr, uint16_t ep_mps, uint8_t ep_type);
HAL_StatusTypeDef HAL_PCD_EP_Close(PCD_HandleTypeDef* hpcd, uint8_t ep_addr);
HAL_StatusTypeDef
HAL_PCD_EP_Receive(PCD_HandleTypeDef* hpcd, uint8_t ep_addr, uint8_t* pBuf, uint32_t len);
HAL_StatusTypeDef
  ...
  */
    #define PCD_SET_EPTYPE(USBx, bEpNum, wType) \
        (PCD_SET_ENDPOINT( \
            (USBx), \
            (bEpNum), \
            ((PCD_GET_ENDPOINT((USBx), (bEpNum)) & USB_EP_T_MASK) | (wType) | USB_EP_CTR_TX \
             | USB_EP_CTR_RX) \
        ))
...
\
            ...
            _wRegVal = \
                (__IO \
                     uint16_t*)(_wRegBase + 0x400U + ((((uint32_t)(bEpNum)*8U) + 4U) * PMA_ACCESS)); \
            *_wRegVal = ((wAddr) >> 1) << 1; \
...
\
            ...
            pdwReg = \
                (__IO \
                     uint16_t*)(_wRegBase + 0x400U + ((((uint32_t)(bEpNum)*8U) + 2U) * PMA_ACCESS)); \
...
\
            ...
            _wRegVal = \
                (__IO \
                     uint16_t*)(_wRegBase + 0x400U + ((((uint32_t)(bEpNum)*8U) + 6U) * PMA_ACCESS)); \
