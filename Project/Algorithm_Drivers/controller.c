...
/******************************** FUZZY PID **********************************/
static float FuzzyRuleKpRAW[7][7] = { PB, PB, PM, PM, PS, ZE, ZE, PB, PB, PM, PS, PS, ZE,
                                      PS, PM, PM, PM, PS, ZE, PS, PS, PM, PM, PS, ZE, PS,
                                      PM, PM, PS, PS, ZE, PS, PS, PM, PM, PS, ZE, PS, PM,
                                      PM, PM, PB, ZE, ZE, PM, PM, PM, PB, PB };

static float FuzzyRuleKiRAW[7][7] = { PB, PB, PM, PM, PS, ZE, ZE, PB, PB, PM, PS, PS, ZE,
                                      ZE, PB, PM, PM, PS, ZE, PS, PS, PM, PM, PS, ZE, PS,
                                      PM, PM, PS, PS, ZE, PS, PS, PM, PB, ZE, ZE, PS, PS,
                                      PM, PB, PB, ZE, ZE, PS, PM, PM, PB, PB };

static float FuzzyRuleKdRAW[7][7] = { PS, PS, PB, PB, PB, PM, PS, PS, PS, PB, PM, PM, PS,
                                      ZE, ZE, PS, PM, PM, PS, PS, ZE, ZE, PS, PS, PS, PS,
                                      PS, ZE, ZE, ZE, ZE, ZE, ZE, ZE, ZE, PB, PS, PS, PS,
                                      PS, PS, PB, PB, PM, PM, PM, PS, PS, PB };

void Fuzzy_Rule_Init(
    ...
    if (abs(pid->Err) > pid->DeadBand) {
        if (pid->FuzzyRule == NULL) {
            ...
            if (pid->OLS_Order > 2)
                ...
                pid->Dout =
                    (pid->Kd + pid->FuzzyRule->KdFuzzy) * (pid->Err - pid->Last_Err) / pid->dt;
        }
...
static void f_Trapezoid_Intergral(PID_t* pid) {
    if (pid->FuzzyRule == NULL)
        ...
        pid->ITerm =
            (pid->Ki + pid->FuzzyRule->KiFuzzy) * ((pid->Err + pid->Last_Err) / 2) * pid->dt;
}
...
static void f_Derivative_On_Measurement(PID_t* pid) {
    if (pid->FuzzyRule == NULL) {
        ...
        if (pid->OLS_Order > 2)
            ...
            pid->Dout =
                (pid->Kd + pid->FuzzyRule->KdFuzzy) * (pid->Last_Measure - pid->Measure) / pid->dt;
    }
}
...
float Feedforward_Calculate(Feedforward_t* ffc, float ref) {
    ...
    ffc->Ref =
        ref * ffc->dt / (ffc->LPF_RC + ffc->dt) + ffc->Ref * ffc->LPF_RC / (ffc->LPF_RC + ffc->dt);
...
float LDOB_Calculate(LDOB_t* ldob, float measure, float u) {
    ...
    ldob->Disturbance = ldob->c[0] * ldob->Measure + ldob->c[1] * ldob->Measure_dot
    ...
    ldob->Disturbance =
        float_constrain(ldob->Disturbance, -ldob->Max_Disturbance, ldob->Max_Disturbance);
